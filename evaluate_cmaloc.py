#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Standalone CMA-Loc evaluator for GAGeo.

Metrics:
    Detection: A@0.5:0.95, ACC@0.5, ACC@0.75
    Model mask: mIoU, mDice, AAE, ME
    Optional SAM mask: mIoU, mDice, AAE, ME
    Runtime: average latency per sample

Supported options:
    - splits: val, test, unseen_test
    - grouping by task_type, size_category, and shape_category
...
def load_cfg_with_env(config_path: str) -> Dict[str, Any]:
    ...
    checkpoint_dir = Path(os.environ.get("CHECKPOINT_DIR", ws_dir / "checkpoints_offline")).expanduser()
    defaults = {
        "WORKSPACE_DIR": str(ws_dir),
        "ROOT_DIR": os.environ.get("ROOT_DIR", str(ws_dir.parent)),
        "WORKSPACE_NAME": os.environ.get("WORKSPACE_NAME", ws_dir.name),
        "CHECKPOINT_DIR": str(checkpoint_dir),
        "DATA_ROOT": os.environ.get("DATA_ROOT", str(ws_dir / "data" / "urban")),
        "OUTPUT_ROOT": os.environ.get("OUTPUT_ROOT", str(ws_dir / "outputs")),
    }
    defaults["JSON_ROOT"] = os.environ.get("JSON_ROOT", str(ws_dir / "data" / "json"))
    ...
    def _expand(obj):
        ...
        if isinstance(obj, str):
            ...
            # Checkpoints copied from training machines may leave absolute paths
            # baked into config.yaml. For offline evaluation, resolve
            ...
            for key, default in defaults.items():
                value = value.replace(f"${{{key}}}", default)
            return value
        return obj
...
def decode_segmentation(segmentation, h: int, w: int) -> np.ndarray:
    """Decode COCO segmentation to a binary uint8 mask with shape [H, W]."""
...
class EvalDataset(Dataset):
    """
        Evaluation dataset decoupled from the training dataloader.
        It reads val/test/unseen_test JSON files, supports point/bbox/mask
        prompts, and returns grouping fields for metric breakdowns.
    """
    ...
    def __getitem__(self, idx: int):
        ...
        # Optional pose ground truth: rotation matrix and position.
        ...
        # Resize to img_size and transform coordinates consistently.
        S = self.img_size
...
def evaluate_split(
    ...
    for batch in tqdm(loader, desc="Evaluating", leave=False):
        ...
        if prompt_type == "point":
            ...
            # IMPORTANT:
            # prompt encoder + prompt_coords path expects boxes in (x, y, w, h)
...
def print_grouped(results: OrderedDict, split_name: str, prompt_type: str, has_model_mask=True, has_sam=True):
    print("\n" + "=" * 120)
    print(f"  GAGeo Evaluation — {split_name} | prompt={prompt_type}")
    print("=" * 120)
...
def parse_args():
    ...
    checkpoint_dir = Path(os.environ.get("CHECKPOINT_DIR", ws_dir / "checkpoints_offline")).expanduser()
    p = argparse.ArgumentParser(description="Evaluate GAGeo (grouped metrics)")
    p.add_argument("--config", type=str, default=str(ws_dir / "configs" / "default.yaml"))
    p.add_argument(
        "--checkpoint",
        type=str,
        default=str(ws_dir / "GAGeo_ckpt" / "gageo" / "mp_rank_00_model_states.pt"),
    )
...
def main():
    ...
    os.environ.setdefault("WORKSPACE_DIR", str(ws_dir))
    os.environ.setdefault("DATA_ROOT", str(ws_dir / "data" / "urban"))
    os.environ.setdefault("JSON_ROOT", str(ws_dir / "data" / "json"))
    os.environ.setdefault("OUTPUT_ROOT", str(ws_dir / "outputs"))
    ...
    # Load SAM only when explicitly needed. Model-mask metrics do not require it.
    sam_predictor = None
    ...
    json_root = Path(os.environ.get("JSON_ROOT", str(ws_dir / "data" / "json"))).expanduser()
    split_to_json = {
        "val": "val_all.json",
        "test": "test_all.json",
    ...
    for split in args.splits:
        ...
        ds = EvalDataset(data_list, image_root=image_root, img_size=img_size)
        loader = DataLoader(
            ds,
