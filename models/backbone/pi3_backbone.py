...
class MaskedFlashAttentionRope(nn.Module):
    ...
    def forward_qkv(self, x_q, x_kv, qpos=None, kvpos=None, attn_mask=None):
        ...
        Used by global attention where K includes front+mask while Q/V use original tokens.
        """
        from torch.nn.functional import scaled_dot_product_attention
        from torch.nn.attention import SDPBackend
...
class Pi3Backbone(nn.Module):
    ...
    def _build_local_attn_mask(
        ...
        """
        Local stage uses a masked GLOBAL self-attention over all tokens:
    ...
    def decode_with_extra_tokens(
        ...
        Args:
            hidden: [B*N, hw, C] encoded visual features
        ...
        # Pre-compute the global attention mask when enabled.
        global_mask = None
