# Cross-view localization with a unified backbone.
#
# Main design:
# - Prompt tokens and learnable queries are injected directly into Pi3 Backbone
# - Custom attention masks control token interactions
...
from .backbone.pi3_backbone import Pi3Backbone
...
class CrossViewLocalizer(nn.Module):
    """
    Cross-View Drone Localization with Unified Backbone.
    ...
    3. Pi3 Backbone: Process all tokens together with custom attn masks
    ...
    def __init__(
        ...
        # Note: uses dec_embed_dim (1024) not output_dim (2048) because tokens
...
def build_cross_view_localizer(
    ...
    **kwargs
    ...
    """
    Build CrossViewLocalizer with optional pretrained weights.
    ...
    Args:
        ...
        **kwargs: Additional model arguments for CrossViewLocalizer
    
    Returns:
        CrossViewLocalizer model
    """
    model = CrossViewLocalizer(freeze_backbone=freeze_backbone, **kwargs)
    
    if pretrained_pi3 is not None:
        _load_pi3_weights(model.backbone, pretrained_pi3)
...
def _load_pi3_weights(backbone: Pi3Backbone, checkpoint_path: str):
    ...
    print(f"Loaded Pi3 weights into backbone from {checkpoint_path}")
    print(f"  Loaded keys: {len(filtered)}")
    # Expected missing: task tokens/projections and masked_blocks wrappers
    # that share decoder parameters.
