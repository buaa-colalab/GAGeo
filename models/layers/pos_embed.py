...
class PositionGetter(object):
    ...
    def __call__(self, b, h, w, device):
        if not (h,w,device) in self.cache_positions:
            x = torch.arange(w, device=device)
            y = torch.arange(h, device=device)
            self.cache_positions[h,w,device] = torch.cartesian_prod(y, x) # (h, w, 2)
        pos = self.cache_positions[h,w,device].view(1, h*w, 2).expand(b, -1, 2).clone()
        return pos